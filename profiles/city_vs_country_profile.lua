...
restrictions = Sequence {
  "motorcycle", "motor_vehicle", "vehicle", }
...
function handle_speed(way,result,data)
  if result.city_forward_speed == -1 then
    ...
    local country_highway_speed = country_speed_profile[data.highway]
...
function handle_side_roads(way,result)  
  ...
  if "yes" == sideway or
  "rotary" == sideway then
    result.city_forward_speed = result.city_forward_speed * side_road_speed_multiplier
    result.city_backward_speed = result.city_backward_speed * side_road_speed_multiplier
    result.country_forward_speed = result.country_forward_speed * side_road_speed_multiplier
    result.country_backward_speed = result.country_backward_speed * side_road_speed_multiplier
  end
end
...
function handle_speed_scaling(way,result)
  ...
  if result.city_forward_speed > 0 or result.city_backward_speed > 0 then
  ...
  if result.city_forward_speed > 0 then
    local scaled_speed = result.city_forward_speed * speed_reduction
  ...
  if result.city_backward_speed > 0 then
    local scaled_speed = result.city_backward_speed * speed_reduction
    local penalized_speed = math.huge
    if service and service_speeds[service]then
      penalized_speed = service_speeds[service]
    elseif width <= 3 or (lanes <= 1 and is_bidirectional) then
  ...
  if result.country_forward_speed > 0 or result.country_backward_speed > 0 then
    local width_string = way:get_value_by_key("width")
    if width_string and tonumber(width_string:match("%d*")) then
      width = tonumber(width_string:match("%d*"))
    end
  ...
  local service = way:get_value_by_key("service")
  if result.country_forward_speed > 0 then
    local scaled_speed = result.country_forward_speed * speed_reduction
    local penalized_speed = math.huge
    if service and service_speeds[service] then
      penalized_speed = service_speeds[service]
    elseif width <= 3 or (lanes <= 1 and is_bidirectional) then
      penalized_speed = result.country_forward_speed / 2
    end
    result.country_forward_speed = math.min(penalized_speed, scaled_speed)
  end

  if result.country_backward_speed > 0 then
    local scaled_speed = result.country_backward_speed * speed_reduction
    local penalized_speed = math.huge
    if service and service_speeds[service]then
      penalized_speed = service_speeds[service]
    elseif width <= 3 or (lanes <= 1 and is_bidirectional) then
      penalized_speed = result.country_backward_speed / 2
    end
    result.country_backward_speed = math.min(penalized_speed, scaled_speed)
  end
...
function handle_maxspeed(way,data,result)
  ...
  if forward and forward > 0 then
    result.city_forward_speed = forward
    result.country_forward_speed = forward
  end

  if backward and backward > 0 then
    result.city_backward_speed = backward
    result.country_backward_speed = backward
  end
end
...
function handle_alternating_speed(way,result)
  if "alternating" == way:get_value_by_key('oneway') then
    ...
    if result.city_forward_speed ~= math.huge then
      result.city_forward_speed = result.city_forward_speed * scaling_factor
    end
    if result.country_forward_speed ~= math.huge then
      result.country_forward_speed = result.country_forward_speed * scaling_factor
    end
    if result.city_backward_speed ~= math.huge then
      result.city_backward_speed = result.city_backward_speed * scaling_factor
    end
    if result.country_backward_speed ~= math.huge then
      result.country_backward_speed = result.country_backward_speed * scaling_factor
    end
  end
end
