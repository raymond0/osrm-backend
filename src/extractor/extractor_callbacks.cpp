...
void ExtractorCallbacks::ProcessWay(const osmium::Way &input_way, const ExtractionWay &parsed_way)
{
    if ((parsed_way.forward_travel_mode == TRAVEL_MODE_INACCESSIBLE ||
         parsed_way.city_forward_speed <= 0) &&
    ...
    if (parsed_way.forward_travel_mode != TRAVEL_MODE_INACCESSIBLE)
    {
        BOOST_ASSERT(parsed_way.duration > 0 || parsed_way.city_forward_speed > 0);
        forward_duration_data =
            toValueByEdgeOrByMeter(parsed_way.duration, parsed_way.city_forward_speed / 3.6);
    ...
    {
        BOOST_ASSERT(parsed_way.duration > 0 || parsed_way.city_backward_speed > 0);
        backward_duration_data =
            toValueByEdgeOrByMeter(parsed_way.duration, parsed_way.city_backward_speed / 3.6);
        // fallback to duration as weight
        if (fallback_to_duration)
        {
            BOOST_ASSERT(false);
